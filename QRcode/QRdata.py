...
class QRdata:
    def __init__(self, version, dtype, errlvl):
        ...
        # Calculate the number of message bits and how it is to be split in blocks
    ...
    def encode(self, msg_str):
        ...
        ind = self.encode_msg()   
        if ind == -1:  
            print("Error encoding the message. Aborting...")
            return        
        
        # Pad the data starting at the last index 
        self.pad_data(ind)
        
        # Split the data into blocks 
        self.msg_blocks, self.ec_blocks = self.split_data() 
        ...
        data_bytelist = self.interlace_blocks() 
    ...
    def lookup_dataspec(self):
        ...
        Tthe next two integers are the corresponding quantities for blocks of type 2 (if applicable)
        """
    ...
    def encode_msg(self):
        match self.dtype:
            case 0:
                # Add a check here to ensure that the message is numeric! 
                ind = self.encode_numeric()
            case 1:
                # Need to convert to uppercase since alphanumeric encoding assumes only uppercase characters
                self.msg = self.msg.upper()
                ind = self.encode_alphanumeric()                
            case 2:
                ind = self.encode_binary()
            case _:
                ind = -1         
    ...
    def encode_numeric(self):
        ...
        # ADD EXCEPTION HANDLING FOR NON-NUMERIC CHARACTERS HERE!!!
    ...
    def encode_alphanumeric(self):
        ...
        for i in range(self.msglen):
            ...
            # The function alphanum() returns -1 if a character cannot be encoded
            if alphanum_list[i] == -1:
                # ADD EXCEPTION HANDLING FOR NON-ALPHANUMERIC CHARACTERS HERE!!!
                return -1
